...
spincore.impulse_builder(
    num_channels=4,
    channel_numbers=[1, 3, 4, 2],   # 1 — твой первый (жёлтый), 3 — второй (розовый)
    impulse_counts=[3, 1,2,1],    # по 3 импульса на каждый (включая дубли границ)
    start_times=[
        0,      10200, 40400,    # Канал 1 — импульс 0–40200 + "фиксация" в 10200 (пересечение)
        10200,                  # Канал 3 — импульс 10200–40200, потом пауза
        0, 40400,
        80400
    ],
    stop_times=[
        40200, 40200, 80400,     # Канал 1 — "удерживает" активное состояние, пока не кончится цикл
        40200,      # Канал 3 — аналогично, но без влияния на первый
        10000, 50400,
        80500
    ],
